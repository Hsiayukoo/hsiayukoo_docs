...
![image](https://github.com/user-attachments/assets/70fc932c-12ae-498e-bf86-0f3c3233691f)


- 我们通过 mindocr 来 **提供各种模型服务化部署的 docker file**。
- mindocr 提供给外部 **k8s 的配置**，方便客户进行服务化部署。（类似 torch serve）

那么关键就是做到哪一步，我们对标 TorchServe。torchserve 提供 AKS 、EKS、GKE 的配置模板，**那么我们当然优先支持 CCE ，也就是华为云的。**

TorchServe 的 kubernetes 目录结构如下
...
我们认为 ocrServing 是工具，不是包，不应该被直接导入。整体逻辑结构如下（这里只绘制**单台服务器上单任务实例部署方式**，生产环境部署结合 k8s_support 里提供的 k8s 配置文件 和 docker 里面提供的 dockerfile 进行部署）：

![image](https://github.com/user-attachments/assets/f803f6a9-c402-482a-9ca8-fe92e9e0dae9)


我们希望在 mindocr 里面嵌入 ocrServing 到 deploy 目录下。

![image](https://github.com/user-attachments/assets/0746eecc-9b03-4222-a8d7-c6cec92b97e2)
